  ...
  describe('clearAuthToken', () => {
    ...
    it('clears the session token on clearAuthToken', async () => {
      ...
      await store.dispatch(login('test', 'test')).then(() => {
        expect(sessionStorage.get(AUTH_TOKEN_KEY)).toBe('TestToken');
        expect(localStorage.get(AUTH_TOKEN_KEY)).toBe(undefined);
        clearAuthToken();
        expect(sessionStorage.get(AUTH_TOKEN_KEY)).toBe(undefined);
        expect(localStorage.get(AUTH_TOKEN_KEY)).toBe(undefined);
      });
    });
    it('clears the local storage token on clearAuthToken', async () => {
      ...
      await store.dispatch(login('user', 'user', true)).then(() => {
        expect(sessionStorage.get(AUTH_TOKEN_KEY)).toBe(undefined);
        expect(localStorage.get(AUTH_TOKEN_KEY)).toBe('TestToken');
        clearAuthToken();
        expect(sessionStorage.get(AUTH_TOKEN_KEY)).toBe(undefined);
        expect(localStorage.get(AUTH_TOKEN_KEY)).toBe(undefined);
      });
